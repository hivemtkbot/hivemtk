...
const fetchAll = async () => {
  ...
  if (!token) {
    ElMessage.warning(i18n.global.t('请先登录，获取JWT后再访问账号/规则/日志'))
    return
  }
  const res = await xianyuAutoReplyApi.listAccounts()
...
const saveAccount = async () => {
  ...
  if (!cookie) {
    ElMessage.warning(i18n.global.t('请先在新标签页完成登录，然后从浏览器开发者工具中复制Cookie并粘贴到上方输入框'))
    return
  }
...
const handleDelete = async (row) => {
  const token = localStorage.getItem('token')
  if (!token) { ElMessage.warning(i18n.global.t('请先登录后再删除')); return }
  try {
...
const saveRule = async () => {
  const token = localStorage.getItem('token')
  if (!token) { ElMessage.warning(i18n.global.t('请先登录后再保存规则')); return }
  try {
    const ruleData = { ...rule.value }
    if (!ruleData.is_rag_enabled) {
      ruleData.rag_product_id = null
    }
    await xianyuAutoReplyApi.saveRule({ ...ruleData, platform })
...
const start = async () => {
  const token = localStorage.getItem('token')
  if (!token) { ElMessage.warning(i18n.global.t('请先登录后再启动')); return }
  try {
...
const stop = async () => {
  const token = localStorage.getItem('token')
  if (!token) { ElMessage.warning(i18n.global.t('请先登录后再停止')); return }
  try {
