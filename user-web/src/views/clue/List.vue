  ...
  <div class="dashboard">
    <div class="dashboard-content">
      ...
      <div style="margin-bottom: 20px; text-align: right;">
        <el-button type="primary" @click="openImportDialog">{{ $t('导入线索') }}</el-button>
      </div>
      ...
      <el-dialog v-model="importDialogVisible" :title="$t('导入线索')" width="600px">
        <el-form ref="importForm" :model="importForm" label-width="80px">
          <el-form-item :label="$t('线索类型')" prop="type">
            <el-select v-model="importForm.type" :placeholder="$t('请选择线索类型')" value-key="value" @change="handleTypeChange">
...
const deleteClue = async (clue) => {
  ElMessageBox.confirm(`确定要删除线索 ${clue.name} 吗？`, '警告', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning', }).then(async () => {
    // 更新任务状态
    const res = await clueApi.delete(clue.id)
    // 拦截器已解包，res 直接就是数据对象
    ElMessage({ type: 'success', message: i18n.global.t('线索已删除'), })
    fetchCluelist()
  })
}
