    ...
    <el-card>
      <template #header>
        <div class="card-header">
          <span>{{ $t('短信草稿') }}</span>
          <el-button type="primary" @click="handleCreateDraft" :loading="loading">{{ $t('新建草稿') }}</el-button>
        </div>
      </template>
...
const rules = {
  title: [
    ...
    { min: 1, max: 100, message: i18n.global.t('标题长度在 1 到 100 个字符'), trigger: 'blur' }
  ],
  content: [
...
const handleConfirmSend = async () => {
  ...
  sending.value = true
  try {
    await smsApi.sendDraft(currentDraft.value.id, { phone: sendForm.phone })
    ElMessage.success(i18n.global.t('短信发送成功'))
    sendDialogVisible.value = false
  } catch (error) {
    ElMessage.error(i18n.global.t('短信发送失败'))
  } finally {
    sending.value = false
  }
}
