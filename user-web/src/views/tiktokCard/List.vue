<template>
  <div class="tiktok-card-container">
    <el-card class="box-card">
      <template #header>
        <div class="card-header">
          <span>TikTok卡片管理</span>
    ...
    >
      <el-descriptions v-if="currentCard" :column="2" border>
        ...
        <el-descriptions-item label="平台">TikTok</el-descriptions-item>
...
import TikTokCardPreview from '@/components/TikTokCardPreview.vue'
...
const dialogTitle = ref('添加TikTok卡片')
...
const getList = async () => {
  ...
  try {
    ...
    const res = await getTikTokCardList(params)
    // 拦截器已解包，res 直接就是数据对象
...
const handleEdit = async (row) => {
  dialogTitle.value = '编辑TikTok卡片'
  try {
    const res = await getTikTokCard(row.id)
    // 拦截器已解包，res 直接就是数据对象
...
const handleDelete = (row) => {
  ElMessageBox.confirm('确定要删除该TikTok卡片吗？', '提示', {
    confirmButtonText: '确定',
    cancelButtonText: '取消',
    type: 'warning'
  }).then(async () => {
    try {
      const res = await deleteTikTokCard(row.id)
      // 拦截器已解包，成功即执行后续逻辑
  ...
  formRef.value.validate(async (valid) => {
    if (valid) {
      try {
        ...
        if (form.id) {
          // 更新
          res = await updateTikTokCard(form)
        } else {
          // 创建
          res = await createTikTokCard(form)
        }
...
const resetForm = () => {
  ...
  form.description = ''
  form.image_url = ''
  form.redirect_url = 'https://www.tiktok.com'
  form.tags = ''
  form.is_active = true
...
const handleStats = (row) => {
  router.push(`/tiktok-card-stats/${row.id}`)
}

// 跳转到统计页面
const goToStats = () => {
  router.push('/tiktok/stats')
}
  ...
  try {
    ...
    if (row.short_link_url) {
      ...
      try {
        ...
        // 生成短链接失败，使用原始链接
        linkUrl = row.redirect_url || window.location.origin + '/tiktok/card/' + row.id
      }
    }
