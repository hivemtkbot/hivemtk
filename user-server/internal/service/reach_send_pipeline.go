...
func (l *MemorySendRateLimiter) shardOf(key string) *rateLimiterShard {
	var h uint32
	for i := 0; i < len(key); i++ {
		h = h*31 + uint32(key[i])
	}
...
func (l *MemorySendRateLimiter) Reset(key string) {
	s := l.shardOf(key)
	s.mu.Lock()
...
func (a *DefaultContentAuditor) ensureCompiled() {
	a.mu.Lock()
	defer a.mu.Unlock()
	if !a.dirty {
		return
...
func (a *DefaultContentAuditor) AddSensitiveWord(words ...string) {
	a.mu.Lock()
...
func (a *DefaultContentAuditor) AddAdLawKeyword(words ...string) {
	a.mu.Lock()
...
func (a *acAutomaton) match(content string) []string {
	var hits []string
...
// Entries 返回所有审计日志
func (l *MemorySendAuditLogger) Entries() []*SendAuditEntry {
	l.mu.Lock()
	defer l.mu.Unlock()
...
// SetCost 设置渠道单价
func (t *MemorySendCostTracker) SetCost(channel string, cost float64) {
	t.mu.Lock()
...
// Balance 返回当前余额
func (t *MemorySendCostTracker) Balance() float64 {
	t.mu.Lock()
	defer t.mu.Unlock()
	return t.balance
}
...
// TotalUsed 返回累计消费
func (t *MemorySendCostTracker) TotalUsed() float64 {
	t.mu.Lock()
	defer t.mu.Unlock()
	return t.totalUsed
}
...
// isRetryable 是否可重试
func (p *defaultSendPipeline) isRetryable(err error, retryableErrors []string) bool {
	if err == nil {
		return false
	}
...
func (p *defaultSendPipeline) computeBackoff(policy SendRetryPolicy, attempt int) time.Duration {
...
// Stats 返回统计快照
func (p *countedSendPipeline) Stats() SendPipelineStats {
	p.mu.RLock()
	defer p.mu.RUnlock()
	return p.stats
}
...
// Count 返回调用次数
func (m *FuncChannelAdapter) Count() int32 {
...
// Count 返回调用次数
func (a *AlwaysFailAdapter) Count() int32 {
...
// Count 返回调用次数
func (a *FlakyAdapter) Count() int32 {
	return atomic.LoadInt32(&a.CallCnt)
}
