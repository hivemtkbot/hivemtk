...
func TestHumanizeEvalService_Evaluate_RetryPass(t *testing.T) {
	...
	svc := newServiceWithStubs(rule, nil, nil, repo, nil)
	// 重生成回调：返回新的回复
	svc.WithRegenerateFn(func(ctx context.Context, input *dto.HumanizeEvalInput, feedback *dto.HumanizeEvalResult) (string, error) {
...
func TestDecideLowQualitySampleType_NaturalnessLow(t *testing.T) {
	...
	got := svc.decideLowQualitySampleType(result)
...
func TestDecideLowQualitySampleType_NilResult(t *testing.T) {
	svc := &HumanizeEvalService{}
	got := svc.decideLowQualitySampleType(nil)
	if got != "retry_exhausted" {
		t.Errorf("type=%q want retry_exhausted", got)
	}
}
...
func TestHumanizeEvalService_Chaining(t *testing.T) {
	...
	svc.WithThreshold(0.90).
		WithSampleRate(0.20).
		WithBoundary(0.65, 0.80).
		WithMaxRetry(5)
...
func TestHumanizeEvalService_Chaining_Invalid(t *testing.T) {
	...
	// threshold 无效
	svc.WithThreshold(0).WithThreshold(-0.1).WithThreshold(1.5)
	// sampleRate 无效
	svc.WithSampleRate(-0.1).WithSampleRate(1.5)
	// boundary 无效（low < 0, high <= low, high > 1）
	svc.WithBoundary(-0.1, 0.5).WithBoundary(0.5, 0.5).WithBoundary(0.5, 1.5)
	// maxRetry 无效
	svc.WithMaxRetry(0).WithMaxRetry(-1)
...
func TestHumanizeEvalService_PersistCalled(t *testing.T) {
	...
	if repo.lastResult == nil {
		t.Error("lastResult 不应为 nil")
	}
}
