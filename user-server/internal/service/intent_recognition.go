...
func (s *IntentRecognizer) SetSOPService(svc *SOPService) {
	s.sopService = svc
}
...
func (s *IntentRecognizer) Recognize(ctx context.Context, sessionID, customerID, text string) (*dto.RecognizeResult, error) {
	...
	// 1. 规则匹配（O(1) 字典）
	if r := s.recognizeByRule(text); r != nil {
...
func (s *IntentRecognizer) recognizeByRule(text string) *dto.RecognizeResult {
...
func (s *IntentRecognizer) GetIntentStats(days int) (map[string]int, error) {
	if s.db == nil {
...
func (s *IntentRecognizer) GetRecentIntents(customerID string, limit int) ([]model.IntentRecord, error) {
	if s.db == nil {
		return nil, nil
	}
	var records []model.IntentRecord
