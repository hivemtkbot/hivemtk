...
func (s *RagMetricsService) Start() {
	s.mu.Lock()
	if s.started {
		s.mu.Unlock()
		return
...
// flushLoop 后台定时刷写
func (s *RagMetricsService) flushLoop() {
	defer s.wg.Done()
...
func (s *RagMetricsService) RecordQuery(req *RecordQueryRequest) {
	if s == nil || req == nil {
		return
	}
	if s.db == nil {
...
func (s *RagMetricsService) buildQueryLog(req *RecordQueryRequest) *model.RagQueryLog {
...
func (s *RagMetricsService) flush() error {
	s.mu.Lock()
	if len(s.queue) == 0 {
		s.mu.Unlock()
		return nil
...
// Start 启动 cron
func (c *RagMetricsCron) Start() {
	c.wg.Add(1)
	go c.run()
}

// Stop 停止 cron
func (c *RagMetricsCron) Stop() {
	close(c.stopCh)
	c.wg.Wait()
}

// run 定时循环
func (c *RagMetricsCron) run() {
	defer c.wg.Done()
