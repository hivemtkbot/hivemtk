...
type ShortLinkService interface {
	Create(req *dto.CreateShortLinkRequest) (*dto.ShortLinkResponse, error)
	Update(req *dto.UpdateShortLinkRequest) (*dto.ShortLinkResponse, error)
	Delete(id uint) error
	GetByID(id uint) (*dto.ShortLinkResponse, error)
	GetByShortCode(shortCode string) (*dto.ShortLinkResponse, error)
	GetList(req *dto.ListShortLinkRequest) (*dto.ShortLinkListResponse, error)
	AccessShortLink(req *dto.AccessShortLinkRequest) (*dto.AccessShortLinkResponse, error)
	GenerateShortCode(req *dto.GenerateShortCodeRequest) (*dto.GenerateShortCodeResponse, error)
	GetStats(req *dto.ShortLinkStatsRequest) (*dto.ShortLinkStatsResponse, error)
	GetAllStats(req *dto.AllShortLinksStatsRequest) (*dto.AllShortLinksStatsResponse, error)
	ShareShortLink(req *dto.ShareShortLinkRequest) (*dto.ShareShortLinkResponse, error)
}
...
// Create 创建短链
func (s *shortLinkService) Create(req *dto.CreateShortLinkRequest) (*dto.ShortLinkResponse, error) {
	// 检查短码是否已存在
	existingLink, _ := s.shortLinkRepo.GetByShortCode(req.ShortCode)
	if existingLink != nil {
	...
	if req.DomainID > 0 {
		domain, err := s.domainRepo.GetByID(int(req.DomainID))
		if err != nil {
	...
	err := s.shortLinkRepo.Create(shortLink)
	if err != nil {
		return nil, err
	}
...
// Update 更新短链
func (s *shortLinkService) Update(req *dto.UpdateShortLinkRequest) (*dto.ShortLinkResponse, error) {
	// 获取现有短链
	shortLink, err := s.shortLinkRepo.GetByID(req.ID)
	if err != nil {
	...
	err = s.shortLinkRepo.Update(shortLink)
	if err != nil {
		return nil, err
	}
...
// Delete 删除短链
func (s *shortLinkService) Delete(id uint) error {
	_, err := s.shortLinkRepo.GetByID(id)
	if err != nil {
		return errors.New("短链不存在")
	}

	return s.shortLinkRepo.Delete(id)
}

// GetByID 根据ID获取短链
func (s *shortLinkService) GetByID(id uint) (*dto.ShortLinkResponse, error) {
	shortLink, err := s.shortLinkRepo.GetByID(id)
	if err != nil {
		return nil, errors.New("短链不存在")
	}
...
func (s *shortLinkService) GetByShortCode(shortCode string) (*dto.ShortLinkResponse, error) {
	shortLink, err := s.shortLinkRepo.GetByShortCode(shortCode)
	if err != nil {
		return nil, errors.New("短链不存在")
	}
...
// GetList 获取短链列表
func (s *shortLinkService) GetList(req *dto.ListShortLinkRequest) (*dto.ShortLinkListResponse, error) {
	// 设置默认分页参数
	if req.Page <= 0 {
		req.Page = 1
	}
	...
	shortLinks, total, err := s.shortLinkRepo.GetList(req.Page, req.PageSize, req.ShortCode, req.OriginalURL, req.Status)
	if err != nil {
		return nil, err
	}
...
func (s *shortLinkService) AccessShortLink(req *dto.AccessShortLinkRequest) (*dto.AccessShortLinkResponse, error) {
	// 根据短码获取短链
	shortLink, err := s.shortLinkRepo.GetByShortCode(req.ShortCode)
	if err != nil {
		return nil, errors.New("短链不存在")
	}
	...
	// 增加点击次数
	err = s.shortLinkRepo.IncreaseClickCount(shortLink.ID)
	if err != nil {
		// 即使增加点击次数失败，也不影响访问
	...
	err = s.accessRepo.Create(accessRecord)
	if err != nil {
		// 即使记录访问信息失败，也不影响访问
...
func (s *shortLinkService) GenerateShortCode(req *dto.GenerateShortCodeRequest) (*dto.GenerateShortCodeResponse, error) {
	// 设置默认长度
	if req.Length <= 0 {
		req.Length = 6
	}
	...
	for i := 0; i < maxAttempts; i++ {
		shortCode := s.generateRandomString(req.Length, charset)

		// 检查短码是否已存在
		_, err := s.shortLinkRepo.GetByShortCode(shortCode)
		if err != nil {
			// 短码不存在，可以使用
...
func (s *shortLinkService) generateRandomString(length int, charset string) string {
...
// GetStats 获取短链统计
func (s *shortLinkService) GetStats(req *dto.ShortLinkStatsRequest) (*dto.ShortLinkStatsResponse, error) {
	// 获取短链信息
	shortLink, err := s.shortLinkRepo.GetByID(req.ID)
	if err != nil {
		return nil, errors.New("短链不存在")
	...
	if startDate.IsZero() && endDate.IsZero() {
		// 如果没有指定日期范围，获取总访问量
		_, totalAccessCount, err := s.accessRepo.GetByShortLinkID(req.ID, 1, 0)
		if err != nil {
			totalAccess = 0
		} else {
		...
		// 获取指定日期范围内的访问量
		dailyStats, err := s.accessRepo.GetDailyStatsByShortLinkID(req.ID, startDate, endDate)
		if err == nil {
	...
	todayStats, err := s.accessRepo.GetDailyStatsByShortLinkID(req.ID, todayStart, todayEnd)
	...
	// 获取设备类型统计
	deviceTypeStats, err := s.accessRepo.GetDeviceTypeStatsByShortLinkID(req.ID, startDate, endDate)
...
func (s *shortLinkService) GetAllStats(req *dto.AllShortLinksStatsRequest) (*dto.AllShortLinksStatsResponse, error) {
	// 解析日期
	var startDate, endDate time.Time
	var err error
	...
	// 获取总访问量
	var totalAccess int64
	dailyStats, err := s.accessRepo.GetAllDailyStats(startDate, endDate)
	if err == nil {
		for _, stat := range dailyStats {
			if count, ok := stat["count"].(int64); ok {
				totalAccess += count
	...
	todayStats, err := s.accessRepo.GetAllDailyStats(todayStart, todayEnd)
	var todayAccess int64
	if err == nil && len(todayStats) > 0 {
	...
	// 获取设备类型统计
	deviceTypeStats, err := s.accessRepo.GetAllDeviceTypeStats(startDate, endDate)
	var deviceStats []dto.DeviceTypeStats
	if err == nil {
		for _, stat := range deviceTypeStats {
			deviceType, _ := stat["device_type"].(string)
	...
	// 获取各短链基本统计
	shortLinksStats, err := s.accessRepo.GetAllShortLinksBasicStats(startDate, endDate)
...
// ShareShortLink 分享短链
func (s *shortLinkService) ShareShortLink(req *dto.ShareShortLinkRequest) (*dto.ShareShortLinkResponse, error) {
	// 获取短链信息
	shortLink, err := s.shortLinkRepo.GetByID(req.ID)
	if err != nil {
		return nil, errors.New("短链不存在")
