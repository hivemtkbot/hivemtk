...
func (s *xiaohongshuCardService) Update(ctx context.Context, req *dto.XiaohongshuCardUpdateRequest) (*dto.XiaohongshuCardResponse, error) {
	...
	if originalDomainPoolID != req.DomainPoolID {
		...
		// 重新获取卡片信息以获取短链
		updatedCard, err = s.repo.GetByID(updatedCard.ID)
		if err != nil {
			return nil, err
		}
	}
...
func (s *xiaohongshuCardService) GenerateShortLink(ctx context.Context, card *model.XiaohongshuCard) error {
	...
	shortLinkResp, err := s.shortLinkService.Create(createReq)
	if err != nil {
		return err
	}

	// 更新卡片的短链ID
	err = s.repo.UpdateShortLinkID(card.ID, &shortLinkResp.ID)
	if err != nil {
		return err
	}
...
func (s *xiaohongshuCardService) toResponse(card *model.XiaohongshuCard) *dto.XiaohongshuCardResponse {
	// 获取短链信息
	shortCode := ""
	shortLinkURL := ""
	if card.ShortLinkID != nil {
...
func (s *xiaohongshuCardService) toResponseWithShortLink(card *model.XiaohongshuCard, shortCode string) *dto.XiaohongshuCardResponse {
	shortLinkURL := ""
	if shortCode != "" {
		shortLinkURL = "/s/" + shortCode
	}
