...
func (s *kuaishouCardService) Create(ctx context.Context, req *dto.KuaishouCardCreateRequest) (*dto.KuaishouCardResponse, error) {
	...
	// 重新获取卡片信息以获取短链
	card, err = s.repo.GetByID(createdCard.ID)
	if err != nil {
		return nil, err
	}
...
func (s *kuaishouCardService) Update(ctx context.Context, req *dto.KuaishouCardUpdateRequest) (*dto.KuaishouCardResponse, error) {
	...
	// 重新获取卡片信息以获取短链
	card, err = s.repo.GetByID(updatedCard.ID)
	if err != nil {
		return nil, err
	}
...
func (s *kuaishouCardService) GetList(ctx context.Context, req *dto.KuaishouCardListRequest) (*dto.KuaishouCardListResponse, error) {
	...
	for _, card := range cards {
		...
		responseList = append(responseList, *s.toResponseWithShortLink(&card, shortCode))
	}
...
func (s *kuaishouCardService) LikeCard(ctx context.Context, id uint) error {
	// 增加点赞数
	err := s.repo.IncrementLikeCount(id)
	if err != nil {
		return err
	}
...
func (s *kuaishouCardService) ViewCard(ctx context.Context, id uint) error {
	// 增加浏览数
	_, err := s.repo.IncrementViewCount(id)
	if err != nil {
		return err
	}
...
func (s *kuaishouCardService) toResponse(card *model.KuaishouCard) *dto.KuaishouCardResponse {
	// 获取短链信息
	shortCode := ""
	shortLinkURL := ""
	if card.ShortLinkID != nil && *card.ShortLinkID != 0 {
...
func (s *kuaishouCardService) toResponseWithShortLink(card *model.KuaishouCard, shortCode string) *dto.KuaishouCardResponse {
	shortLinkURL := ""
	if shortCode != "" {
		shortLinkURL = "/s/" + shortCode
	}
