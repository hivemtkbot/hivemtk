...
func (s *SalesPersonaService) ensureReposFromDB() {
	if s.db == nil {
		return
	}
	if s.repo == nil {
...
func (s *SalesPersonaService) BuildReport(staffID uint) (*PersonaReport, error) {
	s.ensureReposFromDB()
	...
	avgResponseSec, _ := s.repo.AvgHumanResponseSec()
	respScore := 100.0
	...
	// 2. 转化能力 (订单数 / 会话数)
	sessions, orders, _ := s.repo.SessionOrderCount(staffID)
	convScore := 0.0
	if sessions > 0 {
	...
	// 3. SOP 使用率
	sopCount, _ := s.repo.CountSopExecutionsByStaff(staffID)
	...
	// 4. 知识库调用
	ragCount, _ := s.repo.CountRagQueryLogsByUser(staffID)
	...
	// 5. 异议处理
	objCount, _ := s.repo.CountObjectionRecordsByStaff(staffID)
	...
	satStats, _ := s.repo.SatisfactionByStaff(staffID)
	if satStats == nil {
	...
	// 7. 跟进及时性 (会话关单率)
	closeStats, _ := s.repo.SessionCloseByStaff(staffID)
	if closeStats == nil {
...
// ListStaffs 列出有数据的员工
func (s *SalesPersonaService) ListStaffs() ([]map[string]any, error) {
	s.ensureReposFromDB()
	rows, err := s.repo.ListTopStaffsBySession()
	if err != nil {
		return nil, err
	}
