...
func TestAccountService_CreateAccount_WithHeadlessSettings(t *testing.T) {
	setupAccountServiceTestDB(t)
	...
	account := model.Account{
		TgName:              "headless_account",
		TgBotToken:          "123456:ABC-DEF1234ghIkl-zyx57W2v1u123ew11",
		GroupID:             123456789,
		Price:               "100.00",
		URL:                 "http://example.com",
		DouyinHeadless:      &enableHeadless,
		KuaishouHeadless:    &enableHeadless,
		XiaohongshuHeadless: &disableHeadless,
		XianyuHeadless:      &enableHeadless,
	...
	if result.DouyinHeadless == nil || !*result.DouyinHeadless {
		t.Error("Expected DouyinHeadless to be true")
	}

	if result.XiaohongshuHeadless != nil && *result.XiaohongshuHeadless {
		t.Error("Expected XiaohongshuHeadless to be false")
	}
}
...
// TestAccountService_CreateAccount_WithAllHeadlessTrue 测试所有无头模式设置为 true
func TestAccountService_CreateAccount_WithAllHeadlessTrue(t *testing.T) {
	setupAccountServiceTestDB(t)
	...
	account := model.Account{
		...
		DouyinHeadless:      &enableHeadless,
		KuaishouHeadless:    &enableHeadless,
		XiaohongshuHeadless: &enableHeadless,
		XianyuHeadless:      &enableHeadless,
	...
	if result.KuaishouHeadless == nil || !*result.KuaishouHeadless {
		t.Error("Expected KuaishouHeadless to be true")
	}
	if result.XiaohongshuHeadless == nil || !*result.XiaohongshuHeadless {
		t.Error("Expected XiaohongshuHeadless to be true")
	}
	if result.XianyuHeadless == nil || !*result.XianyuHeadless {
		t.Error("Expected XianyuHeadless to be true")
	}
	if result.TiktokHeadless == nil || !*result.TiktokHeadless {
		t.Error("Expected TiktokHeadless to be true")
	}
}

// TestAccountService_CreateAccount_WithAllHeadlessFalse 测试所有无头模式设置为 false
func TestAccountService_CreateAccount_WithAllHeadlessFalse(t *testing.T) {
	setupAccountServiceTestDB(t)
	...
	account := model.Account{
		TgName:              "all_headless_false",
		TgBotToken:          "123456:ABC-DEF1234ghIkl-zyx57W2v1u123ew11",
		GroupID:             123456789,
		Price:               "100.00",
		URL:                 "http://example.com",
		DouyinHeadless:      &disableHeadless,
		KuaishouHeadless:    &disableHeadless,
		XiaohongshuHeadless: &disableHeadless,
		XianyuHeadless:      &disableHeadless,
		TiktokHeadless:      &disableHeadless,
	}
	...
	if result.DouyinHeadless != nil && *result.DouyinHeadless {
		t.Error("Expected DouyinHeadless to be false")
	}
	if result.KuaishouHeadless != nil && *result.KuaishouHeadless {
		t.Error("Expected KuaishouHeadless to be false")
	}
	if result.XiaohongshuHeadless != nil && *result.XiaohongshuHeadless {
		t.Error("Expected XiaohongshuHeadless to be false")
	}
	if result.XianyuHeadless != nil && *result.XianyuHeadless {
		t.Error("Expected XianyuHeadless to be false")
	}
	if result.TiktokHeadless != nil && *result.TiktokHeadless {
		t.Error("Expected TiktokHeadless to be false")
	}
}

// TestAccountService_CreateAccount_WithNilHeadless 测试 nil 无头模式设置（使用默认值）
func TestAccountService_CreateAccount_WithNilHeadless(t *testing.T) {
	setupAccountServiceTestDB(t)

	service := NewAccountService()

	account := model.Account{
		TgName:     "nil_headless_account",
		TgBotToken: "123456:ABC-DEF1234ghIkl-zyx57W2v1u123ew11",
		GroupID:    123456789,
		Price:      "100.00",
		URL:        "http://example.com",
	...
	// 默认值应该为 true
	if result.DouyinHeadless == nil || !*result.DouyinHeadless {
		t.Error("Expected DouyinHeadless default to be true")
	}
	if result.KuaishouHeadless == nil || !*result.KuaishouHeadless {
		t.Error("Expected KuaishouHeadless default to be true")
	}
	if result.XiaohongshuHeadless == nil || !*result.XiaohongshuHeadless {
		t.Error("Expected XiaohongshuHeadless default to be true")
	}
	if result.XianyuHeadless == nil || !*result.XianyuHeadless {
		t.Error("Expected XianyuHeadless default to be true")
	}
	if result.TiktokHeadless == nil || !*result.TiktokHeadless {
		t.Error("Expected TiktokHeadless default to be true")
	}
}
