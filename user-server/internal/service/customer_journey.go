...
// GetState 获取客户旅程状态
func (s *CustomerJourneyService) GetState(customerID string) *JourneyState {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
// Touch 记录互动（不改变阶段）
func (s *CustomerJourneyService) Touch(customerID, source string) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *CustomerJourneyService) Transition(ctx context.Context, customerID string, toStage JourneyStage, source, operatorID, reason string, metadata map[string]any) (*JourneyEvent, error) {
	...
	// 触发的副作用
	s.applyStageSideEffects(state, toStage)
	s.mu.Unlock()
	// 通知订阅者
...
func (s *CustomerJourneyService) applyStageSideEffects(state *JourneyState, stage JourneyStage) {
...
func (s *CustomerJourneyService) isValidStage(stage JourneyStage) bool {
...
// AddSubscriber 添加订阅者
func (s *CustomerJourneyService) AddSubscriber(sub JourneySubscriber) {
	s.subscribers = append(s.subscribers, sub)
}

// ListByStage 按阶段列出客户
func (s *CustomerJourneyService) ListByStage(stage JourneyStage) []string {
	s.mu.RLock()
	defer s.mu.RUnlock()
	customerIDs := []string{}
...
func (s *CustomerJourneyService) AutoDetectSleeping() []string {
	s.mu.Lock()
	defer s.mu.Unlock()
	wokeUp := []string{}
	now := time.Now()
	for cid, state := range s.states {
		...
		if now.Sub(ref) > threshold {
			...
			state.StageHistory = append(state.StageHistory, *event)
			s.applyStageSideEffects(state, StageSleeping)
...
func (s *CustomerJourneyService) GetOverview() *JourneyOverview {
	s.mu.RLock()
	defer s.mu.RUnlock()
