...
func (s *RFMCalculatorService) CalculateRFM(userID uint, rule *model.RFMRule) (*model.UserRFM, error) {
	// 获取用户统计信息
	...
	// 计算 R、F、M 得分
	rScore := s.calcRScore(stats.LastTransactionAt, rule)
	fScore := s.calcFScore(stats.TransactionCount, rule)
	mScore := s.calcMScore(stats.TotalAmount, rule)

	totalScore := rScore + fScore + mScore

	// 确定用户分层
	layer := s.determineLayer(rScore, fScore, mScore, stats.LastTransactionAt)
...
func (s *RFMCalculatorService) getUserStats(userID uint) (*UserStats, error) {
	tgID := int64(userID)
	orders, err := s.orderRepo.GetByTgID(tgID)
	if err != nil {
		return nil, err
	}
...
func (s *RFMCalculatorService) calcRScore(lastTransactionAt *time.Time, rule *model.RFMRule) int {
...
func (s *RFMCalculatorService) calcFScore(transactionCount int, rule *model.RFMRule) int {
...
// totalAmount 单位：分
func (s *RFMCalculatorService) calcMScore(totalAmount int64, rule *model.RFMRule) int {
	if totalAmount == 0 {
		return 1
	}
...
func (s *RFMCalculatorService) determineLayer(rScore, fScore, mScore int, lastTransactionAt *time.Time) model.RFMLayer {
	// 检查是否为新用户
...
func (s *RFMCalculatorService) CalculateAllUsers(ctx context.Context) (int, error) {
	// 获取活跃规则
	rule, err := s.rfmRuleRepo.GetActiveRule()
	if err != nil {
		// 如果没有活跃规则，使用默认规则
		rule = s.getDefaultRule()
	}

	// Get all users with transactions
	// In production, this would query users who have placed orders
	users, err := s.getAllUserIDs()
	if err != nil {
		return 0, err
	}
	...
	for _, userID := range users {
		rfm, err := s.CalculateRFM(userID, rule)
		if err != nil {
			continue
		}

		// 保存或更新 RFM 记录
		existing, _ := s.userRfmRepo.GetByUserID(userID)
		if existing != nil {
			rfm.ID = existing.ID
			err = s.userRfmRepo.Update(rfm)
		} else {
			err = s.userRfmRepo.Create(rfm)
		}
...
func (s *RFMCalculatorService) getDefaultRule() *model.RFMRule {
...
func (s *RFMCalculatorService) getAllUserIDs() ([]uint, error) {
	tgIDs, err := s.orderRepo.GetDistinctPaidTgIDs()
	if err != nil {
		return nil, err
	}
...
func (s *RFMCalculatorService) GetRFMRule() (*model.RFMRule, error) {
	return s.rfmRuleRepo.GetActiveRule()
}

// DeleteRFMRule 删除 RFM 规则
func (s *RFMCalculatorService) DeleteRFMRule(id uint) error {
	return s.rfmRuleRepo.Delete(id)
}

// SaveRFMRule 保存 RFM 规则
func (s *RFMCalculatorService) SaveRFMRule(req *SaveRFMRuleRequest) (*model.RFMRule, error) {
	rule := &model.RFMRule{
		Name:     req.Name,
		RDays1:   req.RDays1,
		RDays2:   req.RDays2,
...
func (s *RFMCalculatorService) UpdateRFMRule(id uint, req *SaveRFMRuleRequest) (*model.RFMRule, error) {
	rule, err := s.rfmRuleRepo.GetByID(id)
	if err != nil {
	...
	if err := s.rfmRuleRepo.Update(rule); err != nil {
		return nil, err
	}
...
func (s *RFMCalculatorService) GetRFMStats() (map[string]any, error) {
	layerCount, err := s.userRfmRepo.GetLayerCount()
	if err != nil {
		return nil, err
	}
...
func (s *RFMCalculatorService) GetUsersByLayer(layer string, page, pageSize int) ([]*UserRFMWithUser, int64, error) {
	rfms, total, err := s.userRfmRepo.GetByLayer(layer, page, pageSize)
	if err != nil {
		return nil, 0, err
	}

	return s.enrichUserData(rfms), total, nil
}

// GetRFMList 获取用户 RFM 列表
func (s *RFMCalculatorService) GetRFMList(page, pageSize int) ([]*UserRFMWithUser, int64, error) {
	rfms, total, err := s.userRfmRepo.GetAll(page, pageSize)
	if err != nil {
		return nil, 0, err
	}
...
func (s *RFMCalculatorService) GetUserRFM(userID uint) (*UserRFMWithUser, error) {
	rfm, err := s.userRfmRepo.GetByUserID(userID)
	if err != nil {
		return nil, err
	}

	result := s.enrichUserData([]*model.UserRFM{rfm})
	if len(result) == 0 {
...
func (s *RFMCalculatorService) enrichUserData(rfms []*model.UserRFM) []*UserRFMWithUser {
