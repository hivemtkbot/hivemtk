...
// Key 计算唯一键字符串
func (k ConversationKey) Key() string {
...
func (s *InboxService) UpsertFromHubMessage(ctx context.Context, msg *model.MessageHub) (*model.InboxConversation, error) {
	...
	if err := s.db.Where("platform = ? AND account_id = ? AND customer_id = ?", msg.Platform, msg.AccountID, cid).First(&conv).Error; err != nil {
		return nil, err
	}
	return &conv, nil
}
...
func (s *InboxService) List(ctx context.Context, q InboxQuery) ([]*model.InboxConversation, int64, error) {
	...
	tx := s.db.Model(&model.InboxConversation{})
...
func (s *InboxService) AutoAssign(ctx context.Context, conversationID uint, candidates []string, operatorID string) (*model.InboxAssignment, error) {
	...
	staff, err := s.pickStaff(candidates)
	if err != nil {
		return nil, err
	}
...
func (s *InboxService) RoundRobinAssign(ctx context.Context, conversationID uint, candidates []string, operatorID string) (*model.InboxAssignment, error) {
	...
	staff, err := s.pickRoundRobin(candidates)
	if err != nil {
		return nil, err
	}
	return s.Assign(ctx, InboxAssignRequest{
...
// StaffLoad 客服当前负载
func (s *InboxService) StaffLoad(staffUserID string) (int, error) {
	if s.db == nil {
		return 0, nil
	}
	var n int64
...
// pickStaff 选择负载最小的客服
func (s *InboxService) pickStaff(candidates []string) (string, error) {
	if len(candidates) == 0 {
...
func (s *InboxService) pickRoundRobin(candidates []string) (string, error) {
	if len(candidates) == 0 {
		return "", ErrInboxInvalidAssignTo
	}
	type c struct {
...
func (s *InboxService) addLoad(staff string) {
	if staff == "" {
		return
	}
	s.mu.Lock()
...
func (s *InboxService) releaseLoad(staff string) {
	if staff == "" {
		return
	}
	s.mu.Lock()
