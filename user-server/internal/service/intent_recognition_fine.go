...
func (s *IntentRecognizer) RecognizeIntent(ctx context.Context, message, customerID, sessionID string) (*IntentResult, error) {
	...
	// 1. 规则匹配
	ruleResult := s.recognizeFineByRule(message)
...
func (s *IntentRecognizer) recognizeFineByRule(text string) *IntentResult {
...
//   - major: 大类过滤（可选，空表示不限）
//   - limit: 返回条数上限
func (s *IntentRecognizer) GetIntentLogs(customerID, major string, limit int) ([]model.IntentLog, error) {
	if s.db == nil {
		return nil, nil
	}
...
// 参数：
//   - days: 统计最近 N 天的数据
func (s *IntentRecognizer) GetIntentLogStats(days int) (map[string]any, error) {
	if s.db == nil {
		return nil, nil
	}
	if days <= 0 {
...
func (s *IntentRecognizer) QueryIntentLogsByTraceID(traceID string) ([]model.IntentLog, error) {
	if s.db == nil || traceID == "" {
		return nil, nil
	}
	var logs []model.IntentLog
