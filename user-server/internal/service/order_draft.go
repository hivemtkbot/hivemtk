...
// ----------------------------------------------------------------------------
// P1-CLOSE-11：订单意向提取 → 自动生成订单草稿（销售一键确认）
//
// 商业市场需求：销售每天接触 50+ 客户，AI 谈单过程中会捕获到"我要买 XX
// 套餐 3 次 2280 元"等关键信息。原实现：只记录"提取了订单意向"的动作，
// 但销售仍需手动打开订单系统、查找产品、填写数量/价格 → 必然遗漏。
//
// 修复：意向提取后自动生成"订单草稿"，销售只需在"待确认草稿"列表里
// 一键确认即可下单，4 件事自动发生：
...
// CreateFromIntent 从订单意向自动生成草稿（P1-CLOSE-11 核心入口）
...
// Confirm 销售一键确认草稿 → 创建正式订单（P1-CLOSE-11 核心入口）
