...
func (r *PersonaEvaluationResult) ScoreByDimension(dim PersonaDimension) (float64, bool) {
...
// Threshold 设置阈值（链式）
func (e *LLMPersonaEvaluator) WithThreshold(t float64) *LLMPersonaEvaluator {
	if t > 0 && t <= 1 {
		e.threshold = t
	}
	return e
...
func (e *RuleBasedPersonaEvaluator) WithThreshold(t float64) *RuleBasedPersonaEvaluator {
	if t > 0 && t <= 1 {
		e.threshold = t
	}
	return e
...
func (e *RuleBasedPersonaEvaluator) scoreNaturalness(input *PersonaEvaluationInput) float64 {
	reply := input.AIReply
	score := 0.88 // 基础分
	// AI 痕迹词（每个 -0.3）
	aiTraces := []string{
...
func (e *RuleBasedPersonaEvaluator) scoreRelevance(input *PersonaEvaluationInput) float64 {
...
func (e *RuleBasedPersonaEvaluator) scorePersona(input *PersonaEvaluationInput) float64 {
	score := 0.75 // 基础分
	reply := input.AIReply
...
func (e *RuleBasedPersonaEvaluator) scoreEmotion(input *PersonaEvaluationInput) float64 {
	reply := input.AIReply
...
//   - 180-300 字 → 0.25
//   - >300 字 → 0.1
func (e *RuleBasedPersonaEvaluator) scoreConciseness(input *PersonaEvaluationInput) float64 {
...
func (e *RuleBasedPersonaEvaluator) scoreCompliance(input *PersonaEvaluationInput) float64 {
	reply := input.AIReply
	score := 1.0
	// 广告法极限词（每个 -0.4）
	adLawWords := []string{
...
// WithThreshold 设置阈值
func (s *PersonaEvaluationService) WithThreshold(t float64) *PersonaEvaluationService {
	if t > 0 && t <= 1 {
		s.threshold = t
	}
	return s
...
func (s *PersonaEvaluationService) WithMaxRetry(n int) *PersonaEvaluationService {
	if n > 0 {
		s.maxRetry = n
	}
	return s
...
func (s *PersonaEvaluationService) WithSampleCollector(c LowQualitySampleCollector) *PersonaEvaluationService {
	if c != nil {
		s.sampleCollector = c
	}
	return s
