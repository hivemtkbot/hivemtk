...
func (s *AutoReplyService) GetMerchantHeadlessSettings() (*MerchantHeadlessSettings, error) {
	var merchantAccount model.Account
	err := s.db.First(&merchantAccount).Error
...
func (s *AutoReplyService) SetMerchantHeadless(platform string, headless bool) error {
...
func defaultMerchantAccount() model.Account {
	return model.Account{
		...
		EpayKey:             "default_key",
		EpayPayType:         "alipay",
		EpayQueryUrl:        "https://api.example.com/query",
...
func (s *AutoReplyService) SaveRuleDTO(req AutoReplyRuleSaveReq) error {
...
func (s *AutoReplyService) UpsertAutoReplyAccount(req AutoReplyAccountCreateReq) (uint, error) {
...
func (s *AutoReplyService) GetAutoReplyStatistics(platform string, userID uint) (gin.H, error) {
	var ruleCount int64
