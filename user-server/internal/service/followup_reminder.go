...
func (s *FollowUpService) SetDashboard(d *SalesDashboard) {
	s.dashboard = d
}
...
// Complete 完成跟进
func (s *FollowUpService) Complete(reminderID string) error {
	return s.CompleteWithResult(reminderID, FollowUpResultContacted, "")
}
...
//
// 返回更新后的跟进详情
func (s *FollowUpService) CompleteWithResult(reminderID string, result FollowUpResult, note string) error {
	s.mu.Lock()
	...
	if s.journey != nil {
		s.journey.Touch(customerID, "followup")
		if info, ok := FollowUpResultInfo[result]; ok {
			_, _ = s.journey.Transition(context.Background(), customerID, info.TargetStage, "followup", ownerID,
	...
	// 2) 实时更新销售仪表盘
	if s.dashboard != nil {
		s.dashboard.RecordFollowUp(FollowUpEvent{
			CustomerID: customerID,
			OwnerID:    ownerID,
			Result:     string(result),
			OccurredAt: now,
...
// Cancel 取消跟进
func (s *FollowUpService) Cancel(reminderID string) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	r, ok := s.reminders[reminderID]
	if !ok {
...
func (s *FollowUpService) ListPending(ownerID string, limit int) []*Reminder {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
// ListOverdue 列出已逾期
func (s *FollowUpService) ListOverdue(ownerID string) []*Reminder {
	s.mu.RLock()
	defer s.mu.RUnlock()
	now := time.Now()
...
func (s *FollowUpService) GetDailyCalendar(ownerID string, date time.Time) []*Reminder {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *FollowUpService) GetWeeklyCalendar(ownerID string, startOfWeek time.Time) map[string][]*Reminder {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *FollowUpService) AutoScheduleByJourney(ctx context.Context, customerID, ownerID string) (*Reminder, error) {
	...
	state := s.journey.GetState(customerID)
