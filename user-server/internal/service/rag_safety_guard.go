...
//
// 与 rag_alert.go（系统级指标预警）的关系：
//   - rag_alert.go  关注系统侧：低召回率 / 高延迟 / 嵌入失败率 / 空命中
//   - 本文件       关注内容侧：敏感词 / 广告法 / 竞品 / 画像越权
//
// 职责:
//  1. 敏感词检测：政治/暴力/色情/违禁品等高危词拦截
//  2. 广告法合规：《广告法》绝对化用语（最/第一/国家级）识别与提醒
//  3. 竞品拦截：配置的竞品词命中后拒绝输出并告警
//  4. 用户画像越权：RAG 检索时仅能命中当前用户所属租户的私有知识库
//
...
func (s *RagSafetyGuardService) GetLexicon() SafetyLexicon {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return SafetyLexicon{
...
func (s *RagSafetyGuardService) SetLexicon(lex SafetyLexicon) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *RagSafetyGuardService) AddSensitiveWord(word string) error {
...
func (s *RagSafetyGuardService) AddCompetitorWord(word string) error {
	word = strings.TrimSpace(word)
	if word == "" {
...
// LastUpdate 词库最后更新时间
func (s *RagSafetyGuardService) LastUpdate() time.Time {
	s.mu.RLock()
	defer s.mu.RUnlock()
	return s.updatedAt
}
...
func (s *RagSafetyGuardService) Check(ctx context.Context, req *SafetyCheckRequest) (*SafetyCheckResult, error) {
	...
	// 1) 敏感词检测（最高优先级）
	if issues := s.checkSensitive(req.Content); len(issues) > 0 {
		result.Issues = append(result.Issues, issues...)
	}

	// 2) 广告法合规
	if issues := s.checkAdCompliance(req.Content); len(issues) > 0 {
		result.Issues = append(result.Issues, issues...)
	}

	// 3) 竞品拦截
	if issues := s.checkCompetitor(req.Content); len(issues) > 0 {
		result.Issues = append(result.Issues, issues...)
	}

	// 4) 画像越权：过滤跨租户片段
	if len(req.Sources) > 0 {
		if issues := s.checkPersonaAuthz(req); len(issues) > 0 {
			result.Issues = append(result.Issues, issues...)
		}
	}
	...
	if result.Blocked {
		result.Passed = false
		result.ReplacedContent = s.safeReplacement()
	} else {
		result.Passed = true
	}
...
func (s *RagSafetyGuardService) safeReplacement() string {
...
func (s *RagSafetyGuardService) checkSensitive(content string) []SafetyIssue {
	if content == "" {
		return nil
	}
	lex := s.GetLexicon()
...
func (s *RagSafetyGuardService) checkAdCompliance(content string) []SafetyIssue {
	if content == "" {
		return nil
	}
	lex := s.GetLexicon()
...
//
// 命中后：block（保护租户商业利益）
func (s *RagSafetyGuardService) checkCompetitor(content string) []SafetyIssue {
	if content == "" {
		return nil
	}
	lex := s.GetLexicon()
...
func (s *RagSafetyGuardService) checkPersonaAuthz(req *SafetyCheckRequest) []SafetyIssue {
	if req.TenantID == "" {
		return nil
	}
	var issues []SafetyIssue
...
func (s *RagSafetyGuardService) FilterSourcesByTenant(sources []SafetySource, tenantID string) ([]SafetySource, int) {
	if tenantID == "" {
		return sources, 0
	}
