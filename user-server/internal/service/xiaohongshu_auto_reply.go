...
func (s *XiaohongshuAutoReplyService) TestMatching(ctx context.Context, platform, message string, userID uint) (*model.AutoReplyRule, error) {
	...
	if err := s.db.Where(ctx, "platform = ? AND user_id = ? AND is_active = ?", platform, userID, true).Find(&rules).Error; err != nil {
		return nil, err
	}
...
func (s *XiaohongshuAutoReplyService) ListAccounts(ctx context.Context, userID uint) ([]model.AutoReplyAccount, error) {
	...
	err := s.db.Where(ctx, "platform = ? AND user_id = ?", "xiaohongshu", userID).Find(&items).Error
	return items, err
}
...
func (s *XiaohongshuAutoReplyService) UpsertAccount(ctx context.Context, a *model.AutoReplyAccount) error {
	...
	err := s.db.Where(ctx, "user_id = ? AND platform = ? AND username = ?", a.UserID, "xiaohongshu", a.Username).First(&existing).Error
	if err == nil {
...
func (s *XiaohongshuAutoReplyService) GetRule(ctx context.Context, userID uint) (*model.AutoReplyRule, error) {
	...
	err := s.db.Where(ctx, "platform = ? AND user_id = ?", "xiaohongshu", userID).Order("id DESC").First(&rule).Error
	if err != nil {
		return nil, err
	}
	return &rule, nil
...
func (s *XiaohongshuAutoReplyService) SaveRule(ctx context.Context, rule *model.AutoReplyRule) error {
	...
	err := s.db.Where(ctx, "platform = ? AND user_id = ?", "xiaohongshu", rule.UserID).First(&existing).Error
	if err == nil {
...
func (s *XiaohongshuAutoReplyService) DeleteAccount(ctx context.Context, id uint, userID uint) error {
	return s.db.Where(ctx, "id = ? AND user_id = ? AND platform = ?", id, userID, "xiaohongshu").Delete(&model.AutoReplyAccount{}).Error
}
