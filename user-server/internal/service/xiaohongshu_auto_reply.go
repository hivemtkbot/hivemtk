...
func (s *XiaohongshuAutoReplyService) GetDB() *gorm.DB { return s.db }
...
func (s *XiaohongshuAutoReplyService) TestMatching(ctx context.Context, platform, message string, userID uint) (*model.AutoReplyRule, error) {
	...
	if err := s.db.Where("platform = ? AND user_id = ? AND is_active = ?", platform, userID, true).Find(&rules).Error; err != nil {
		return nil, err
	}
...
func (s *XiaohongshuAutoReplyService) ListAccounts(userID uint) ([]model.AutoReplyAccount, error) {
	var items []model.AutoReplyAccount
	err := s.db.Where("platform = ? AND user_id = ?", "xiaohongshu", userID).Find(&items).Error
	return items, err
}

func (s *XiaohongshuAutoReplyService) UpsertAccount(a *model.AutoReplyAccount) error {
	var existing model.AutoReplyAccount
	err := s.db.Where("user_id = ? AND platform = ? AND username = ?", a.UserID, "xiaohongshu", a.Username).First(&existing).Error
	if err == nil {
...
func (s *XiaohongshuAutoReplyService) SaveCookies(id uint, cookie string, userID uint) error {
...
func (s *XiaohongshuAutoReplyService) GetRule(userID uint) (*model.AutoReplyRule, error) {
	var rule model.AutoReplyRule
	err := s.db.Where("platform = ? AND user_id = ?", "xiaohongshu", userID).Order("id DESC").First(&rule).Error
	if err != nil {
		return nil, err
	}
	return &rule, nil
...
func (s *XiaohongshuAutoReplyService) SaveRule(rule *model.AutoReplyRule) error {
	var existing model.AutoReplyRule
	err := s.db.Where("platform = ? AND user_id = ?", "xiaohongshu", rule.UserID).First(&existing).Error
	if err == nil {
...
func (s *XiaohongshuAutoReplyService) ListRecentLogs(userID uint, page, pageSize int) ([]model.AutoReplyLog, int64, error) {
	...
	s.db.Model(&model.AutoReplyLog{}).
		...
		Count(&total)
	err := s.db.Where("platform = ? AND user_id = ? AND created_at >= ?", "xiaohongshu", userID, cutoff).
...
func (s *XiaohongshuAutoReplyService) StartLoginBrowser(userID uint, username string, accountID uint, headless bool) {
	go func() {
...
func (s *XiaohongshuAutoReplyService) DeleteAccount(id uint, userID uint) error {
	return s.db.Where("id = ? AND user_id = ? AND platform = ?", id, userID, "xiaohongshu").Delete(&model.AutoReplyAccount{}).Error
}
