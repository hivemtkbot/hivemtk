...
func (s *AuthService) JwtUtils() *utils.JWTUtils {
	return s.jwtUtils
}
...
func (s *AuthService) Login(req *LoginRequest) (*LoginResponse, error) {
	...
	err := database.Where("username = ?", req.Username).First(&user).Error
	...
	if errors.Is(err, gorm.ErrRecordNotFound) {
		...
		if err := database.Where("username = ?", req.Username).First(&teamUser).Error; err != nil {
			if errors.Is(err, gorm.ErrRecordNotFound) {
		...
		mfaEnabled, err := mfaSvc.IsMFAEnabled(teamUser.ID)
		if err != nil {
			logger.Errorf("TeamUser MFA 状态查询失败: %v", err)
		}
		if mfaEnabled {
			tempToken, err := mfaSvc.IssueTempToken(teamUser.ID, teamUser.Username, teamUser.Role)
			if err != nil {
				return nil, errors.New("登录失败，请稍后重试")
			}
	...
	return s.loginWithUser(&user)
}

// loginWithUser 使用用户对象完成登录流程
func (s *AuthService) loginWithUser(user *model.SystemUser) (*LoginResponse, error) {
	// 获取数据库连接
	database := db.GetDB()
	...
	mfaSvc := NewMFAService()
	mfaEnabled, err := mfaSvc.IsMFAEnabled(user.ID)
	if err != nil {
		logger.Errorf("MFA 状态查询失败: %v", err)
	}
	if mfaEnabled {
		tempToken, err := mfaSvc.IssueTempToken(user.ID, user.Username, user.Role)
		if err != nil {
			return nil, errors.New("登录失败，请稍后重试")
		}
		return &LoginResponse{
...
func (s *AuthService) GetCurrentUser(userID uint) (*SystemUserResponse, error) {
	// 获取数据库连接
	database := db.GetDB()
...
func (s *AuthService) ChangePassword(userID uint, req *ChangePasswordRequest) error {
	...
	if err := policySvc.ValidatePassword(req.NewPassword, userID); err != nil {
		return err
	}
	...
	// P1-3 修复：记录密码历史
	if err := policySvc.RecordPasswordHistory(userID, req.NewPassword, model.PasswordSourceChangePassword); err != nil {
...
func (s *AuthService) toUserResponse(user *model.SystemUser) *SystemUserResponse {
...
func (s *AuthService) InitChangePassword(username, newPassword string) error {
...
func (s *AuthService) CreateForgotPasswordToken(username string) (string, error) {
	...
	// 清理过期 token
	go s.cleanupExpiredForgotTokens()
...
func (s *AuthService) ResetAdminPasswordWithToken(username, token, newPassword string) error {
	username = strings.TrimSpace(username)
	if username == "" || token == "" {
...
func (s *AuthService) cleanupExpiredForgotTokens() {
	forgotTokenStoreMutex.Lock()
