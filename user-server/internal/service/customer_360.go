...
func (s *Customer360Service) GetCustomer360(userID string) (*Customer360DTO, error) {
	dto := &Customer360DTO{}

	// 1. 获取基本信息和会话统计
	sessions, _, _ := s.sessionRepo.GetByMerchant("", 1, 1000)
	...
	// 2. 填充基本信息
	dto.BasicInfo = s.buildBasicInfo(userSessions)

	// 3. 填充会话统计
	dto.SessionStats = s.buildSessionStats(userSessions)

	// 4. 填充会话历史
	dto.SessionHistory = s.buildSessionHistory(userSessions)

	// 5. 填充消息历史
	dto.MessageHistory, _ = s.buildMessageHistory(userSessions)

	// 6. 填充线索信息
	dto.ClueInfo, _ = s.buildClueInfo(userID)

	// 7. 填充订单信息
	dto.OrderInfo, _ = s.buildOrderInfo(userID)

	// 8. 填充互动统计
	dto.InteractionStats = s.buildInteractionStats(userSessions)

	// 9. 填充用户画像
	dto.UserProfile = s.buildUserProfile(userSessions, dto.InteractionStats, dto.OrderInfo)

	return dto, nil
}

// buildBasicInfo 构建基本信息
func (s *Customer360Service) buildBasicInfo(sessions []*model.CustomerSession) *CustomerBasicInfo {
	if len(sessions) == 0 {
		return nil
	}
...
func (s *Customer360Service) buildSessionStats(sessions []*model.CustomerSession) *SessionStatistics {
	if len(sessions) == 0 {
		return nil
	}
...
func (s *Customer360Service) buildSessionHistory(sessions []*model.CustomerSession) []*SessionHistoryItem {
...
func (s *Customer360Service) buildMessageHistory(sessions []*model.CustomerSession) ([]*MessageHistoryItem, error) {
	history := make([]*MessageHistoryItem, 0)

	for _, session := range sessions {
		messages, _, _ := s.messageRepo.GetBySessionID(session.SessionID, 1, 100)
...
func (s *Customer360Service) buildClueInfo(userID string) (*ClueInfo, error) {
	// 首先获取用户的会话信息来查找联系方式
	sessions, _, err := s.sessionRepo.GetByMerchant("", 1, 1000)
	if err != nil {
		return nil, err
	}
	...
	for _, clueType := range []int64{0, 1, 2, 3, 4, 5} {
		allClues, _, err := s.clueRepo.GetClueAllList(clueType)
		if err == nil {
...
func (s *Customer360Service) buildOrderInfo(userID string) (*OrderInfo, error) {
	...
	allOrders, _, err := s.orderRepo.GetOrderList(1, 10000)
	if err != nil {
		return &OrderInfo{
			Orders: make([]*OrderItem, 0),
		}, nil
...
func (s *Customer360Service) buildInteractionStats(sessions []*model.CustomerSession) *InteractionStats {
...
func (s *Customer360Service) buildUserProfile(sessions []*model.CustomerSession, interactionStats *InteractionStats, orderInfo *OrderInfo) *UserProfile {
	profile := &UserProfile{
		Tags:          make([]string, 0),
...
func (s *Customer360Service) GetCustomerList(page, pageSize int, filters map[string]string) (map[string]*Customer360DTO, int64, error) {
	sessions, total, err := s.sessionRepo.GetByMerchant("", page, pageSize)
	if err != nil {
		return nil, 0, err
	}
	...
	for userID := range userSessions {
		dto, err := s.GetCustomer360(userID)
		if err != nil {
			continue
		}
		result[userID] = dto
...
func (s *Customer360ServiceForTest) GetCustomer360(userID string) (*Customer360DTO, error) {
	...
	return realService.GetCustomer360(userID)
}

// GetCustomerList 获取客户列表（测试版本）
func (s *Customer360ServiceForTest) GetCustomerList(page, pageSize int, filters map[string]string) (map[string]*Customer360DTO, int64, error) {
	realService := &Customer360Service{
		sessionRepo:      s.SessionRepo,
		messageRepo:      s.MessageRepo,
		clueRepo:         s.ClueRepo,
	...
	return realService.GetCustomerList(page, pageSize, filters)
}
