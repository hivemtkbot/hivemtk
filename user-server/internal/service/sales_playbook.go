...
// Add 添加话术
func (s *PlaybookService) Add(entry *PlaybookEntry) (*PlaybookEntry, error) {
	if entry == nil {
...
func (s *PlaybookService) Recommend(req PlaybookQuery) []*PlaybookEntry {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *PlaybookService) RecommendForResponse(industry Industry, productID string, stage JourneyStage, intent string) []*PlaybookEntry {
	// 1) 推断异议类型
	objection := inferObjectionFromIntent(intent)
	return s.Recommend(PlaybookQuery{
		Industry:  industry,
		ProductID: productID,
		Stage:     stage,
		Objection: objection,
...
// RecordUse 记录话术使用 + 结果
func (s *PlaybookService) RecordUse(entryID string, success bool) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
// GetByID 按 ID 查询
func (s *PlaybookService) GetByID(id string) *PlaybookEntry {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
// List 列出所有话术
func (s *PlaybookService) List(industry Industry, stage JourneyStage) []*PlaybookEntry {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *PlaybookService) seedDefaults() {
	// === 医美 行业 ===
...
func (s *PlaybookService) FormatPlaybook(e *PlaybookEntry) string {
