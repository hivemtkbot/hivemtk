...
// cosVec 返回与基准向量 e0 余弦相似度为 sim 的单位查询向量（二维子空间构造）
...
// newCascadeFixture 构造可控级联测试桩：
//   - price_inquiry 全部锚点 = e0（单位向量）
//   - objection_price 全部锚点 = 与 e0 夹角 arccos(objectionCos) 的单位向量
//
// 其余意图示例句未注入 vecs → EmbedOne 返回零向量 → 相似度 0，天然隔离。
...
func TestCascade_EmbeddingLayerSecond(t *testing.T) {
	...
	// 无关键词命中、非示例等值，但向量与 price 锚点同向 → 规则未命中后由 embedding 层采信
...
func TestCascade_LLMFallbackWhenEmbeddingUnavailable(t *testing.T) {
	// embedding 服务不可用（熔断）且无 dispatcher → 兜底 unknown（行为不劣于现状）
	rec, _ := newIntentRecognizerNoDB(t)
...
func TestEmbeddingThreshold_JustBelow075NotTrusted(t *testing.T) {
	rec, stub := newCascadeFixture(t, 0.0) // objection 锚点与 price 正交，排除歧义分支
...
// ---- I-3 歧义缺口 0.05 与 clarify ----

func TestClarify_GapBelow005BothAboveThreshold(t *testing.T) {
	// objection 锚点与 price 锚点近共线（cos=0.99）→ 查询=price 锚点时
	// top1(price)=1.0、top2(objection)=0.99，gap=0.01<0.05 且均过阈 → clarify
...
func TestNoClarify_GapAtOrAbove005(t *testing.T) {
	// cos=0.90 → top1=1.0、top2=0.90，gap=0.10 ≥0.05 → 直接采信 top1
...
func TestUnknownContract_ConstantsAndLowConfDowngrade(t *testing.T) {
	...
	// recognizeByLLM 解析后处理段契约：非 unknown 且 conf<0.7 一律降级 unknown
...
// ---- I-1 EnsureIntentExamplesIndexed（需 PG，环境缺失自动跳过）----
...
func TestEnsureIntentExamplesIndexed_Idempotent(t *testing.T) {
	...
	// pgvector 表硬性 1024 维（bge-m3 对齐），测试向量同维
