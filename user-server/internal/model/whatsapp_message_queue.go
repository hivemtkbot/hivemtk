...
type WhatsAppMessageQueue struct {
	...
	Status    string    `gorm:"type:varchar(20);index" json:"status"` // pending, sent, failed
...
type WhatsAppQueueStatus struct {
	...
	Status    string    `gorm:"type:varchar(20);index" json:"status"` // pending, completed, failed, partial
	CreatedAt time.Time `gorm:"autoCreateTime" json:"created_at"`
	UpdatedAt time.Time `gorm:"autoUpdateTime" json:"updated_at"`
}
