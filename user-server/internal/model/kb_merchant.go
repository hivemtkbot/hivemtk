...
type KnowledgeAPIToken struct {
	...
	Name       string     `gorm:"size:64;not null" json:"name"`           // 描述用途
	Token      string     `gorm:"size:128;uniqueIndex;not null" json:"-"` // 哈希后存储
	TokenPlain string     `gorm:"-" json:"token_plain,omitempty"`         // 仅创建时返回
	Scopes     string     `gorm:"type:jsonb;default:'[\"read\",\"write\"]'" json:"scopes"`
	ProductID  string     `gorm:"size:64;index;not null;default:''" json:"product_id"` // RAG 产品 ID（字符串 UUID）
...
type KnowledgeFeedback struct {
	ID         uint64    `gorm:"primaryKey;autoIncrement" json:"id"`
	ProductID  string    `gorm:"size:64;index;not null;default:''" json:"product_id"` // RAG 产品 ID（字符串）
	...
	Rating     int       `gorm:"default:0" json:"rating"` // 1=helpful 0=neutral -1=bad
...
type ExternalImportJob struct {
	...
	ProductID   string     `gorm:"size:64;index;not null" json:"product_id"` // RAG 产品 ID（字符串）
	Source      string     `gorm:"size:32;not null" json:"source"`           // feishu/notion/dingtalk/custom
