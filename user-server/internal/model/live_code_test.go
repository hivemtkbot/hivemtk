...
func TestLiveCode_GetFullShortLink_WithNilDomain(t *testing.T) {
	liveCode := &LiveCode{
		ShortLink: "abc123",
	}

	link := liveCode.GetFullShortLink()
	if link != "" {
		t.Errorf("Expected empty link with nil ShortDomain, got %s", link)
	}
}

func TestLiveCode_GetFullShortLink_WithHTTP(t *testing.T) {
	liveCode := &LiveCode{
		ShortLink: "abc123",
		ShortDomain: &DomainPool{
			Domain: "example.com",
			Port:   80,
		},
	}
	...
	link := liveCode.GetFullShortLink()
	expected := "http://example.com/abc123"
	if link != expected {
		t.Errorf("Expected link %s, got %s", expected, link)
	}
}

func TestLiveCode_GetFullShortLink_WithHTTPS(t *testing.T) {
	liveCode := &LiveCode{
		ShortLink: "abc123",
		ShortDomain: &DomainPool{
	...
	link := liveCode.GetFullShortLink()
	expected := "https://secure.example.com/abc123"
	if link != expected {
		t.Errorf("Expected link %s, got %s", expected, link)
	}
}

func TestLiveCode_GetFullEntryLink_WithNilDomain(t *testing.T) {
	liveCode := &LiveCode{
		ShortLink: "abc123",
	}

	link := liveCode.GetFullEntryLink()
	if link != "" {
		t.Errorf("Expected empty link with nil EntryDomain, got %s", link)
	}
}

func TestLiveCode_GetFullEntryLink_WithDomain(t *testing.T) {
	liveCode := &LiveCode{
		ShortLink: "abc123",
		EntryDomain: &DomainPool{
	...
	link := liveCode.GetFullEntryLink()
	expected := "http://entry.example.com/entry/abc123"
	if link != expected {
		t.Errorf("Expected link %s, got %s", expected, link)
	}
}

func TestLiveCode_GetFullLandingLink_WithNilDomain(t *testing.T) {
	liveCode := &LiveCode{
		ShortLink: "abc123",
	}

	link := liveCode.GetFullLandingLink()
	if link != "" {
		t.Errorf("Expected empty link with nil LandingDomain, got %s", link)
	}
}

func TestLiveCode_GetFullLandingLink_WithDomain(t *testing.T) {
	liveCode := &LiveCode{
		ShortLink: "abc123",
		LandingDomain: &DomainPool{
	...
	link := liveCode.GetFullLandingLink()
	expected := "https://landing.example.com/landing/abc123"
	if link != expected {
		t.Errorf("Expected link %s, got %s", expected, link)
	}
}
...
func TestLiveCode_GetFullEntryLink_WithHTTPS(t *testing.T) {
	liveCode := &LiveCode{
		ShortLink: "abc123",
		EntryDomain: &DomainPool{
			Domain: "secure-entry.example.com",
			Port:   443,
		},
	}

	link := liveCode.GetFullEntryLink()
	expected := "https://secure-entry.example.com/entry/abc123"
	if link != expected {
		t.Errorf("Expected link %s, got %s", expected, link)
	}
}

func TestLiveCode_GetFullLandingLink_WithHTTP(t *testing.T) {
	liveCode := &LiveCode{
		ShortLink: "abc123",
		LandingDomain: &DomainPool{
			Domain: "landing.example.com",
	...
	link := liveCode.GetFullLandingLink()
	expected := "http://landing.example.com/landing/abc123"
	if link != expected {
		t.Errorf("Expected link %s, got %s", expected, link)
	}
}
