...
// dev 档统一切到 bge-m3 + bge-reranker-v2-m3（与 scripts/inference-host/models.env 一致）
func TestInferenceConfigLoads(t *testing.T) {
	...
	// dev 档 embedding 切到 bge-m3（1024 维），与 pgvector vector(1024) 对齐
	...
	// dev 档 rerank 切到 bge-reranker-v2-m3
	...
	if c.Inference.LLM.Model != "Qwen2.5-1.5B-Instruct" {
		t.Fatalf("llm model 应为 Qwen2.5-1.5B-Instruct（dev 档），实际 %s", c.Inference.LLM.Model)
	}
	...
	// embedding / rerank base_url 必须是宿主机 127.0.0.1，且统一带 /v1 后缀
