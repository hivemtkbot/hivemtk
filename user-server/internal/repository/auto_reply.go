...
func (r *AutoReplyAccountRepository) ListByPlatformAndUser(platform string, userID uint) ([]model.AutoReplyAccount, error) {
...
func (r *AutoReplyAccountRepository) FindByUserAndPlatformAndUsername(userID uint, platform, username string) (*model.AutoReplyAccount, error) {
...
// GetByID 按 ID 获取账号
func (r *AutoReplyAccountRepository) GetByID(id uint) (*model.AutoReplyAccount, error) {
...
// Create 创建账号
func (r *AutoReplyAccountRepository) Create(account *model.AutoReplyAccount) error {
	return r.db.Create(account).Error
}

// Save 保存账号
func (r *AutoReplyAccountRepository) Save(account *model.AutoReplyAccount) error {
	return r.db.Save(account).Error
}

// UpdateCookieByID 按 ID 更新 Cookie 字段
func (r *AutoReplyAccountRepository) UpdateCookieByID(id uint, cookie string) error {
	return r.db.Model(&model.AutoReplyAccount{}).Where("id = ?", id).Update("cookie", cookie).Error
}

// DeleteByIDAndUser 按 ID 与用户删除账号（用户不匹配时不会删除）
func (r *AutoReplyAccountRepository) DeleteByIDAndUser(id, userID uint) error {
	return r.db.Where("id = ? AND user_id = ?", id, userID).Delete(&model.AutoReplyAccount{}).Error
}

// FirstByPlatformAndUser 取平台与用户下的首条账号
func (r *AutoReplyAccountRepository) FirstByPlatformAndUser(platform string, userID uint) (*model.AutoReplyAccount, error) {
	var account model.AutoReplyAccount
...
// NewAutoReplyRuleRepository 创建自动回复规则仓库实例
...
func (r *AutoReplyRuleRepository) FindByPlatformAndUser(platform string, userID uint) (*model.AutoReplyRule, error) {
...
func (r *AutoReplyRuleRepository) FindExistingByPlatformAndUser(platform string, userID uint) (*model.AutoReplyRule, error) {
...
// Create 创建规则
func (r *AutoReplyRuleRepository) Create(rule *model.AutoReplyRule) error {
	return r.db.Create(rule).Error
}

// Save 保存规则
func (r *AutoReplyRuleRepository) Save(rule *model.AutoReplyRule) error {
	return r.db.Save(rule).Error
}

// GetByID 按 ID 获取规则
func (r *AutoReplyRuleRepository) GetByID(id uint) (*model.AutoReplyRule, error) {
	var rule model.AutoReplyRule
...
func (r *AutoReplyRuleRepository) DeleteByID(id uint) error {
	return r.db.Delete(&model.AutoReplyRule{}, id).Error
}

// ListByPlatformAndUserActive 列出平台与用户下启用的规则
func (r *AutoReplyRuleRepository) ListByPlatformAndUserActive(platform string, userID uint) ([]model.AutoReplyRule, error) {
...
func (r *AutoReplyRuleRepository) ListWithFilters(platform string, userID uint, isActive *bool, page, pageSize int) ([]model.AutoReplyRule, int64, error) {
...
func (r *AutoReplyRuleRepository) FindTopDailyLimit(platform string, userID uint) (*model.AutoReplyRule, error) {
	var rule model.AutoReplyRule
	query := r.db.Model(&model.AutoReplyRule{})
	if platform != "" {
		query = query.Where("platform = ?", platform)
...
func (r *AutoReplyRuleRepository) CountByFilters(platform string, userID uint) (int64, error) {
	query := r.db.Model(&model.AutoReplyRule{})
	if platform != "" {
		query = query.Where("platform = ?", platform)
	}
...
// Create 创建日志
func (r *AutoReplyLogRepository) Create(log *model.AutoReplyLog) error {
	return r.db.Create(log).Error
}

// ListRecentByPlatformAndUser 列出平台与用户最近的日志
func (r *AutoReplyLogRepository) ListRecentByPlatformAndUser(platform string, userID uint, page, pageSize int, cutoff time.Time) ([]model.AutoReplyLog, int64, error) {
	if page <= 0 {
		page = 1
	}
	if pageSize <= 0 {
...
func (r *AutoReplyLogRepository) CountByUserAndRuleSince(userID, ruleID uint, since time.Time) (int64, error) {
	var count int64
...
func (r *AutoReplyLogRepository) DeleteSinceByFilters(since time.Time, platform string, userID, accountID uint) error {
...
func (r *AutoReplyLogRepository) CountSinceByFilters(since time.Time, platform string, userID, accountID uint) (int64, error) {
	var count int64
	query := r.db.Model(&model.AutoReplyLog{}).Where("created_at >= ?", since)
	if platform != "" {
		query = query.Where("platform = ?", platform)
...
func (r *AutoReplyLogRepository) CountActiveAccountsSince(since time.Time, platform string, userID uint) (int64, error) {
	var count int64
...
func (r *AutoReplyLogRepository) CountPendingByFilters(platform string, userID uint) (int64, error) {
	var count int64
...
//
// 用于 controller 替代直接调用 GetDB().Model().Count() 的架构违规。
func (r *AutoReplyLogRepository) CountByFilters(platform string, userID uint) (int64, error) {
	var count int64
...
func (r *AutoReplyLogRepository) CountByFiltersAndDate(platform string, userID uint, date string) (int64, error) {
	var count int64
