...
func (m *e2eMockReachAdapter) AccountHealth(_ context.Context, _, _ string) (*AccountHealthInfo, error) {
	return &AccountHealthInfo{}, nil
}
func (m *e2eMockReachAdapter) ListAccounts(_ context.Context, _ string) ([]AccountInfo, error) {
	return nil, nil
}

// makeDeps 创建带 mock adapter 的 deps（SendPipeline 也用 mock adapter 桥接）
func makeDeps(db any, mock *e2eMockReachAdapter) ReachToolDeps {
	deps := ReachToolDeps{Adapter: mock}
	// 重新构造 SendPipeline，让 bridge 使用 mock adapter
	deps.SendPipeline = service.NewSendPipeline(service.DefaultSendPipelineConfig(&reachChannelAdapterBridge{adapter: mock}))
	return deps
}
...
func TestE2E_ReachTelegram_FullPipeline(t *testing.T) {
	...
	deps := makeDeps(db, mock)

	tool := NewReachTelegramSendTool(deps)
	args := map[string]any{
		"account_id": "1",
		"chat_id":    "987654321",
		"content":    "E2E 测试消息",
...
func TestE2E_ReachWhatsApp_FullPipeline(t *testing.T) {
	...
	tool := NewReachWhatsAppSendTool(deps)
	_, err := tool.Execute(context.Background(), map[string]any{
		"account_id":  "10",
...
func TestE2E_ReachFeishu_FullPipeline(t *testing.T) {
	...
	tool := NewReachFeishuSendTool(deps)
	_, err := tool.Execute(context.Background(), map[string]any{
		"account_id": "5",
		"open_id":    "ou_xyz",
		"content":    "E2E 飞书",
