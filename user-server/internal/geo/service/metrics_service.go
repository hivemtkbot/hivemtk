...
func (s *MetricsService) CountTrustSignals(content string) int {
	...
	dataRe := regexp.MustCompile(`\d+%|\d+\.\d+%|约\d+%|超过\d+%|达到\d+%|\d+倍|\d+个|\d+项|\d+次|\d+年|\d+月`)
	caseRe := regexp.MustCompile(`案例[：:][^，。；\n]{5,100}|例如[^，。；\n]{5,100}|以[^，。；\n]{2,30}为例`)
	count := s.countMatches(content, trustSignalRe)
	count += s.countMatches(content, dataRe)
	count += s.countMatches(content, caseRe)
	return count
}
