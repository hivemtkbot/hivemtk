#!/bin/bash
#
# HivemTK User Server - 全面端到端 API 测试脚本 v2
# 正确解析 HTTP 状态码
#
...
test_endpoint() {
    ...
    if [ "$expect_success" = "true" ]; then
        if [ "$http_code" = "200" ]; then
            ...
            # 接受 SUCCESS、0、SUCCESS_CODE 等成功状态码，或空值
...
"
            ...
            SKIPPED=$((SKIPPED + 1))
            echo -e "  ${YELLOW}⏭️  SKIP${NC} [$method] $endpoint (405)"
...
# 主程序
echo "============================================================"
echo "HivemTK User Server - 全面 API 测试 v2"
...
CREATE_CUST=$(curl -s -X POST -H "Authorization: Bearer $TOKEN" -H "Content-Type: application/json" \
    -d '{"name":"full_v2_test","phone":"13400134000","email":"v2_e2e@test.com"}' \
...
for ep in "/api/customer/list" "/api/customer/360/list" "/api/customer-360/list"; do
    test_endpoint "GET" "$ep" "true"
done

if [ -n "$CUSTOMER_ID" ]; then
    for ep in "/api/customer/$CUSTOMER_ID" "/api/customer/360/$CUSTOMER_ID"; do
        test_endpoint "GET" "$ep" "true"
    done
fi
...
test_endpoint "POST" "/api/backups" "true" '{"backup_name":"v2_e2e_backup","backup_type":"full"}'
...
for ep in "/api/system/config" "/api/system/logs" "/api/system/stats" "/api/stats/system" \
          "/api/system/backup" "/api/obs/config" "/api/obs/config/default" "/api/admin/config" \
          "/api/agent/tool-integrations" "/api/agent/settings"; do
    test_endpoint "GET" "$ep" "true"
done
...
echo ""
echo "[9] RAG 知识库..."
for ep in "/api/knowledge/bases" "/api/knowledge/bases/list" "/api/rag/sessions" "/api/rag/metrics"; do
    test_endpoint "GET" "$ep" "true"
done

echo ""
echo "[10] 渠道管理..."
for ep in "/api/whatsapp/accounts" "/api/whatsapp-cloud/accounts" "/api/telegram/accounts" \
          "/api/feishu/accounts" "/api/wecom/accounts" "/api/wechat/accounts" \
          "/api/tiktok/accounts" "/api/douyin/accounts"; do
    test_endpoint "GET" "$ep" "true"
done
...
echo ""
echo "[12] 数据分析..."
for ep in "/api/analytics/funnel" "/api/analytics/ai-productivity" "/api/churn/prediction" \
          "/api/churn/warnings" "/api/churn/statistics" "/api/churn/risk-distribution"; do
    test_endpoint "GET" "$ep" "true"
done
...
echo ""
echo "[15] 内存管理..."
for ep in "/api/memory/short" "/api/memory/long" "/api/memory/context" "/api/memory/list"; do
    test_endpoint "GET" "$ep" "true"
done
...
echo ""
echo "[19] 线索发掘路由..."
for ep in "/api/lead-mining/config"; do
    test_endpoint "GET" "$ep" "true"
done
...
for ep in "/api/notifications" "/api/notifications/unread-count" "/api/accounts/list" \
          "/api/account/list" "/api/quick-replies" "/api/session-tags" \
          "/api/dashboards" "/api/templates" "/api/scripts" "/api/ab-experiments" \
          "/api/integrations" "/api/dashboard/clients" "/api/dashboard/stats" \
          "/api/customer-journey/overview" "/api/objection/categories" \
          "/api/migration/history" "/api/migration/current-version" "/api/events/stats" \
          "/api/community/groups" "/api/wecom/groups" "/api/wecom/customers" \
          "/api/wecom/messages" "/api/shortlink/list" "/api/live-code/list" \
          "/api/email/list" "/api/email/smtp" "/api/clue/list" \
          "/api/dingtalk-app/accounts" "/api/dingtalk/accounts"; do
    test_endpoint "GET" "$ep" "true"
done
