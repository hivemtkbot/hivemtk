...
// 基础配置
const BASE_URL = 'http://localhost:8204/api';
...
async function testApi(method, endpoint, description, expectedStatus = 200, data = null, token = null) {
    totalTests++;
    try {
...
async function runTests() {
    ...
    // 1. 登录获取JWT令牌
    console.log('🔑 用户认证:');
    const loginResult = await testApi('POST', '/auth/login', '用户登录', 200, {
        username: 'admin',
        password: '123456'
    });
    ...
    // 2. 账户管理API测试 - 验证ID验证修复
    console.log('👤 账户管理API测试:');
    
    // 获取账户列表
    const accountsResult = await testApi('GET', '/accounts', '账户列表', 200, null, token);
    ...
    // 3. 抖音卡片API测试 - 验证卡片ID处理优化
    console.log('📱 抖音卡片API测试:');
    
    // 获取卡片列表
    const cardsResult = await testApi('GET', '/douyin-cards', '抖音卡片列表', 200, null, token);
    if (cardsResult.success && cardsResult.data?.data?.length > 0) {
        ...
        if (cardId) {
            ...
            // 测试卡片删除 - 验证ID处理
            await testApi('DELETE', `/douyin-cards/${cardId}`, '卡片删除(ID验证)', 200, null, token);
    ...
    // 4. 短链接API测试 - 验证统计参数处理修复
    console.log('🔗 短链接API测试:');
    
    // 获取短链接列表
    const shortLinksResult = await testApi('GET', '/short-links', '短链接列表', 200, null, token);
    if (shortLinksResult.success && shortLinksResult.data?.data?.length > 0) {
        ...
        if (shortLinkId) {
            // 测试短链接统计 - 验证参数处理修复
            await testApi('GET', `/short-links/${shortLinkId}/stats`, '短链接统计(参数处理修复)', 200, null, token);
    ...
    // 5. 活码管理API测试 - 验证必填字段验证
    console.log('📟 活码管理API测试:');
    
    // 获取活码列表
    const liveCodesResult = await testApi('GET', '/live-codes', '活码列表', 200, null, token);
    ...
    // 测试邮件管理API
    await testApi('GET', '/email/accounts', '邮件账户列表', 200, null, token);
    await testApi('GET', '/email/templates', '邮件模板列表', 200, null, token);
    
    // 测试系统管理API
    await testApi('GET', '/system/logs', '系统日志', 200, null, token);
    await testApi('GET', '/system/stats', '系统统计', 200, null, token);
    await testApi('GET', '/system/config', '系统配置', 200, null, token);
