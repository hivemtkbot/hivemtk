...
| 平台端 | 10 | 见 [`hivemtk-platform/docs/platform-features/`](../../hivemtk-platform/docs/platform-features/README.md) |
| **合计** | **104** | - |

> **统计说明**: 用户端 94 份 + 平台端 10 份 = 104 份功能文档。
...
> 📌 **RAG 三文档边界关系**:`rag-knowledge-base`(配置) → `knowledge-management`(内容入库) → `agent-rag-qa`(应用调用)。三者构成 RAG 完整链路,各司其职。
...
| [email-tracking.md](email-tracking.md) | 邮件追踪（打开/点击像素 + Webhook） | ✅ 已实现 |
| [email-unsubscribe.md](email-unsubscribe.md) | 退订管理 | ✅ 已实现 |
...
| [dingtalk-app-account.md](dingtalk-app-account.md) | 钉钉应用账号管理 | ✅ 已实现 |
...
| [anomaly-login-detector.md](anomaly-login-detector.md) | 异常登录检测 | ✅ 已实现 |

## 十三、安全与权限域

| 文档 | 功能名称 | 状态 |
|------|---------|------|
| [permission-system.md](permission-system.md) | 权限系统（角色/菜单/按钮级） | ✅ 已实现 |
| [row-level-security.md](row-level-security.md) | 行级数据权限（data_scope 中间件） | ✅ 已实现 |
...
| [message-hub.md](message-hub.md) | 消息中心（含 7 大渠道接入总览） | ✅ 已实现 |
...
> 📌 **多 AI 智能体架构**: 一个商户可配置多个独立智能体（销售型/客服型/混合型），每个智能体可绑定不同 LLM、SOP、知识库。详见 [ARCHITECTURE_DIAGRAM.md](../architecture/ARCHITECTURE_DIAGRAM.md)。
...
5. **开源约束**：所有文档不得包含 OTA、License、版本下载、定价、注册开户等已下线功能的描述。
...
- [../architecture/部署方案_用户端.md](../architecture/部署方案_用户端.md) — 用户端部署方案
- [../architecture/ARCHITECTURE_DIAGRAM.md](../architecture/ARCHITECTURE_DIAGRAM.md) — 系统架构图（C4 / 五层 / 模块依赖）
- [../architecture/agent-tools-inventory.md](../architecture/agent-tools-inventory.md) — AI Agent 41 工具注册表清单
