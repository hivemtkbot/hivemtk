...
- 抽象 `DingTalkAppAccount` 模型，承载账号配置 + 入站开关 + 智能体绑定 + 状态字段
- Controller 仅做参数解析与 VO 转换（脱敏）
- Service 承载业务校验与持久化
- 配置测试只做静态校验（必填项检查），真实连通性测试在后续 access_token 刷新时验证
...
| 监控项 | 阈值 | 告警方式 |
|---|---|---|
| last_error_at 持续刷新 | 5 分钟内多次 | 钉钉 |
| inbound_enabled=true 但无 ai_agent_id | 持续 | 邮件（运营提醒） |
