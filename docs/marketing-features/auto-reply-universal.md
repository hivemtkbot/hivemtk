...
1. **账号管理**：每个平台账号独立登录态（Cookie + LocalStorage 持久化）
2. **规则匹配**：关键词模糊匹配 + 置信度评估（规则 > RAG > LLM）
3. **回复执行**：chromedp 找到输入框 → 输入文本 → 发送
4. **日志记录**：每次收发消息入库 + Redis 缓存
5. **速率限制**：防封号策略（同账号 5-15 秒/条）
6. **无头模式**：可切换 headless 节省资源
