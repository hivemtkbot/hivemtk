...
- 每次登录成功 / 失败均落 `login_events`，由 `service.AnomalyLoginDetector` 同步评估风险
- 风险评估命中阈值时写入 `security_alerts`（状态 `open`）
- 管理员通过接口处理（`resolved`）或忽略（`ignored`）告警，并写入审计日志
- 私域独立部署：无 `merchant_id` 字段，按 `user_id` 隔离
