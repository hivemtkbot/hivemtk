...
HiveMTK 智能体平台对外提供 SaaS 服务, 每个租户 (电商品牌/团队) 创建独立的 AI 客服。
不同租户的"知识库" (FAQ / RAG 文档 / SOP 模板) 必须严格隔离, 防止:
- 租户 A 看到租户 B 的私有产品资料
- 跨租户数据泄露 (合规风险, GDPR / 网络安全法)
...
1. **多租户数据隔离**: 严格按 owner_agent_id 隔离, 通过测试矩阵验证
