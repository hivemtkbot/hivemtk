...
### 11.2 可观测性 (私域: 应用层日志 + DB 审计)

| 监控项 | 阈值 | 巡检方式 |
|---|---|---|
| 工具调用失败率 | > 5% | `audit_logs` 表聚合查询 |
| Agent Loop 超时率 | > 10% | `audit_logs` 表聚合查询 |
| 限流触发率 | > 1% | `audit_logs` 表聚合查询 |
| 审计日志溢出 | 接近 10000 | `audit_logs` 表行数查询 (需扩容或持久化) |

> 私域部署: 不接入钉钉/短信/邮件告警通道, 关键指标通过 `scripts/post_deploy_check.sh` 巡检。
